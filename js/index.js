...
if (hacker1.length < hacker2.length){
    ...
    console.log(`You both have equally long names, you have ${hacker1.length} characters.`)
}
...
for (let i = 0; i < shortestName.length; i++){
    if (shortestName === hacker1){
        ...
        if (shortestName[i] < longestName[i] || shortestName.length !== longestName.length){
            message = "The navigator's name goes first."
            break
        } else if (shortestName[i] > longestName[i]) {
            message = "The driver's name goes first."
...
console.log("Alternative recursive solution:", newMessage)