...
for (let i = 0; i < hacker1.length; i++) {
    ...
    console.log(blockNameCaps.slice(0, blockNameCaps.length - 1))
...
for (let i = 0 ; i < loremPara.length; i++) {
    ...
    const et = loremPara[i] + loremPara[i + 1] + loremPara[i + 2] + loremPara[i + 3]
    if (et === " et "){etCount ++}
}
if (wordCount === 1){
...
const pTCCaps = phraseToCheck.toUpperCase()
const pTCReady = pTCCaps.replace(/[^a-zA-Z0-9]/g, '')
let pTCForward = ""
let pTCBackward = ""

console.log(`Ready: ${pTCReady}`)

for (let i = 0; i < pTCReady.length; i++) {
    if (pTCReady[i] === " ") {
        continue
    } else (pTCForward += pTCReady[i])
    }
console.log(`Forward: ${pTCForward}`)
...
console.log(`Backward: ${pTCBackward}`)
if (pTCBackward === pTCForward) {
