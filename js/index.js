...
let reversedPhrase ="";
let revisedPhraseToCheck = "";
let reminder=";"

for (let i = phraseToCheck.length-1; i >= 0; i--) {
    if (phraseToCheck[i] >= ('a' && 'A') && phraseToCheck[i] <= ('z' || 'Z')) {
        reversedPhrase += phraseToCheck[i];
    } else {reminder = phraseToCheck[i]}
}

for (let j = 0; j <= phraseToCheck.length; j++) {
    if (phraseToCheck[j] >= ('a' && 'A') && phraseToCheck[j] <= ('z' || 'Z')) {
        revisedPhraseToCheck += phraseToCheck[j]
    }
}


if (revisedPhraseToCheck.toUpperCase() === reversedPhrase.toUpperCase()) {
    console.log (`Bingo! You've just found a lost Palindrome on your own! Remember that "${phraseToCheck}" is a pretty damn good Palindrome`)
} else console.log (`Not really a Palindrome. Sotades would not be happy with this...`)
