...
//Two index are created, one per each letter to compare, one starts on the beginning a the other by the ending
while (indexStart<indexEnd) {
    let firstChar = phraseToCheck.charAt(indexStart);
    let lastChar = phraseToCheck.charAt(indexEnd);
    //checking if the first element to compare is a letter
    if(firstChar < 'a' || firstChar > 'z'){
        indexStart++;
        continue;
    }
    //checking if the second element to compare is a letter
