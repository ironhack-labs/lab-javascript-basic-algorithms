...
let phraseToCheck = 'A man, a plan, a acanal, Panama!'

let phraseAdapted = phraseToCheck.replace(/[^a-zA-Z0-9 ]/g, '').toLowerCase()
let phraseJoined = phraseAdapted.split(" ").join("")
let phraseReversed = ''

for (let j = phraseJoined.length - 1; j >= 0; j--) {
    phraseReversed += phraseJoined[j]
}

if (phraseReversed === phraseJoined) console.log('Its a palindrome!')
