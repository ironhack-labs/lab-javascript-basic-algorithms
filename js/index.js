...
for (let i = 0; i < testingParagraph.length; i++) {
  if (testingParagraph[i] === " ") numberOfSpaces++;
  if (testingParagraph[i] + testingParagraph[i + 1] === "et") numberOfEts++;
}
...
for (let i = 0; i < phraseToCheck.length; i++) {
  if (
    phraseToCheck.toUpperCase().charCodeAt(i) >= 65 &&
...
for (let i = 0; i < phraseLength; i++) {
  if (
    formattedPhrase[i].toLowerCase() !==
    formattedPhrase[phraseLength - i - 1].toLowerCase()
  ) {
    isPalindrome = false;
    break;
  }
