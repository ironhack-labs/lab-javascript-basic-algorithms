...
var reversedPhrase = reverseStringPal(phraseToCheck);

function reverseStringPal(str) {
  let phraseToReverse = "";
  for (var i = str.length - 1; i >= 0; i--) {
    phraseToReverse += str[i];
  }
...
function cleanString(string) {
  let phraseWithoutPunctuation = removePunctuation(string);
  return removeSpace(phraseWithoutPunctuation);
}

function checkPalindrome(string) {
  let cleanedPhraseToCheck = cleanString(phraseToCheck);
  let cleanedreversedPhrase = cleanString(reversedPhrase);

  if (
    cleanedPhraseToCheck
      .toUpperCase()
      .localeCompare(cleanedreversedPhrase.toUpperCase()) == 0
  ) {
    console.log("9 : This is a palindrome for sure");
  } else {
    console.log("9 : Sorry, this is not a palindrome :(");
  }
}
