...
for (let char of hacker1) {
  printOut += char.toUpperCase() + ' ';
}

console.log(`Hacker's 1 name in CAPS and with Spaces is: ${printOut.trim()}`);
...
console.log(`Hacker's 2 Reversed Name is: ${reverseName} `);
...
console.log(
  `Found the latin word "et" ${
    myParagraphs.split(' ').filter((e) => e === 'et').length
  } times`
);

// Bonus 2

let myString = 'put it up'.replace(/[ ]/g, '').toLowerCase();
